...
import com.wan.yunbrowser.Adapter.HistoryAdapter;
...
public class historyFragment extends Fragment {
	...
	public List<com.wan.yunbrowser.Bean.Bookmark> list;
	public String[] title;
	public String[] url;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
	    ...
	    return rootView;
			...
			try {
				list=stringToJson.getBookmarkList("historys", getData());
			} catch (JSONException e) {
				//Toast.makeText(mContext,"获取历史记录失败，请重试",Toast.LENGTH_SHORT).show();
				Snackbar.make(container,"获取历史记录失败，请重试",Snackbar.LENGTH_SHORT).show();
			}
			adapter=new HistoryAdapter(list, new HistoryAdapter.MyclickListener(){
				@Override
				...
				public void imageclick(final String title){
					...
					builder.setPositiveButton("确定", new OnClickListener()
					{
						...
						public void onClick(DialogInterface dialog, int which)
						{
							...
							map.put("title", title);
							String url1=HttpUtil.BASE_URL+"deleteHistory";
							try {
	...
	public String getData() {
		String url = HttpUtil.BASE_URL + "getHistory" + "?username="
				+ User.username;
		try {
	...
	void refeshView(){
		try {
			...
			Snackbar.make(recyclerView,"获取历史记录失败，请重试",Snackbar.LENGTH_SHORT).show();
		}
		adapter=new HistoryAdapter(list, new HistoryAdapter.MyclickListener(){
			@Override
			public void urlclick(String url) {
